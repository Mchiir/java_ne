...
--    Uses users table.
-- ---------------------------------------------------------------------------
CREATE EXTENSION IF NOT EXISTS pgcrypto; -- Give PostgreSQL responsibility for UUID generation
...
v_user_name TEXT;
    v_period    TEXT;
BEGIN
SELECT full_names INTO v_user_name
FROM users
...
VALUES (
           ...
           NEW.customer_id,
           NEW.id,
           'Dear ' || COALESCE(v_user_name, 'Customer') || ', Your ' || v_period ||
...
--    notify the user. Fires only on the transition to fully-paid.
-- ---------------------------------------------------------------------------
...
BEGIN
    IF NEW.outstanding_balance <= 0
       AND COALESCE(OLD.outstanding_balance, 0) > 0 THEN
...
VALUES (
           ...
           NEW.customer_id,
           NEW.id,
           'Dear ' || COALESCE(v_user_name, 'Customer') ||
